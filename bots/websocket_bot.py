...
class WebSocketBot:
    ...
    def _start(self):
        ...
        if not self.symbols:
            ...
            self.request_coin_list_until_success()   
    ...
    def request_coin_list_until_success(self, retries=5, delay=5):
        for attempt in range(retries):
            if self.symbols:
                break
            self.logger.warning(f"🕒 Attempt {attempt+1}: No symbols yet, retrying in {delay}s...")
            self.request_coin_list()
            time.sleep(delay)

        if not self.symbols:
            self.logger.error("❌ Still no coinlist after retries.")
